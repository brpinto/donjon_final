class StatsController < ApplicationController
  ...
  def index
    ...
    @stats = Stat.find(current_month)
    @users_count = User.count
    @notes_count = Note.count
    @confirmed_count = User.where('confirmed = 1').count
    @notes_per_user = @notes_count / @users_count

    @stats.update(users_total: @users_count, notes_total: @notes_count, users_confirmed: @confirmed_count, notes_per_user: @notes_per_user)
